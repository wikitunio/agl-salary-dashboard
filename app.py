...
# Optional libraries for file parsing (graceful fallback if not installed)
try:
...
def main():
    if check_password():
        ...
        # 1. Hide the default Streamlit popover container initially to prevent flickering
        st.markdown("<style>div[data-testid='stPopover'] { display: none; }</style>", unsafe_allow_html=True)
        
        # 2. Inject heavy CSS to style the chat bubbles, the popup window, and the floating button
        st.markdown("""
        <style>
        /* --- THE FLOATING BUTTON --- */
        ...
        div[data-testid="stPopover"] > button {
            ...
            box-shadow: 0px 8px 25px rgba(0, 102, 255, 0.4) !important;
            transition: transform 0.3s cubic-bezier(0.25, 0.8, 0.25, 1) !important;
        }
        div[data-testid="stPopover"] > button:hover {
            transform: scale(1.1) translateY(-5px) !important;
            box-shadow: 0px 12px 30px rgba(0, 102, 255, 0.6) !important;
        }
        ...
        div[data-testid="stPopover"] > button svg {
            display: none !important; /* Hide Streamlit arrow */
        }
        ...
        div[data-testid="stPopoverBody"] {
            ...
            padding: 0 !important; /* Remove padding for edge-to-edge header */
        ...
        # 3. Create the Popover Container
        with st.popover("🤖"):
            # Header
            st.markdown("""
            <div class="bot-header">
                <h3>🤖 AI Assistant</h3>
                <p>Ask anything about your salary, HR policies, leaves, expenses, or company information.</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            with tool_col1:
                if st.button("🔄 Clear", key="clear_chat", use_container_width=True):
                    ...
                    st.rerun()
            with tool_col2:
                # File uploader wrapped in a small popover to save space
            ...
            if prompt:
                ...
                st.session_state["widget_chat_history"].append({
                    "role": "user", 
                    "content": prompt, 
                    "timestamp": curr_time
                })
            ...
            # Process AI response ONLY IF the last message was from the user
                            ...
                            try:
                                ...
                                if uploaded_file is not None:
                                    ...
                                    if uploaded_file.name.endswith('.csv'):
                                        ...
                                        for i in range(min(3, len(pdf_reader.pages))): # Read first 3 pages
                                ...
                                # Strip out timestamps for the API payload
